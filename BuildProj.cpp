...
float position(Point **building,float *w,float *h,Point p,int n) //position of source p
{
   float len,w1,h1;
   for(int i=0;i<n;i++)
   {
...
float countLen(Point **building,Point p,int n)
{
	float w[n],h[n],temp,len; 
     //store height and weight of building.
	w[0]=0.0;
	h[0]=0.0;
	for(int i=0;i<n;i++)
	{
	   for(int j=0;j<4;j++)
		{
		 ...
		 w[i]= max(w[i],temp); 
		 //maximum weigh value of	 building
		 temp=0.0;
		 temp=building[i][j].y-building[i][j+1].y;
		 h[i]=max(h[i],temp); 
		 //maximum height value of building
		}
	}
	for(int i=0;i<n;i++)
	{
...
int main()
{
	...
	for(int i=0;i<n;i++)
	{
		building1[i]= new struct Point [n];
	}
	...
	cin>>p.y;
	float length=countLen(building1,p,n);
